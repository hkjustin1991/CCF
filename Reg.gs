...
function regEventDateFromKeyUtc_(ev){
  const m = String(ev||'').trim().match(/^SundayService_(\d{4})-(\d{2})-(\d{2})$/);
  if (!m) return null;
  const d = new Date(Date.UTC(parseInt(m[1],10), parseInt(m[2],10)-1, parseInt(m[3],10)));
  return isNaN(d.getTime()) ? null : d;
}
...
function regGetCheckinsDataMinimal_(){
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const sh = ss.getSheetByName('Checkins') || ss.getSheetByName('CHECKINS');
  ...
  const cEv = (col.EventKey !== undefined) ? col.EventKey : 1;
  const cMid = (col.MemberId !== undefined) ? col.MemberId : 2;

  const data = sh.getRange(2,1,lastRow-1,lastCol).getValues();
  const rows = [];

  for (let i=0;i<data.length;i++){
    const ev = String(data[i][cEv]||'').trim();
    if (!ev) continue;
    const mid = String(data[i][cMid]||'').trim().toUpperCase();
    if (!mid) continue;
    rows.push({ eventKey: ev, memberId: mid });
  }
